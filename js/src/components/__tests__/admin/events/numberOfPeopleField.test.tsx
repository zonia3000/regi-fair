...
test('Deleting saved "number of people" field is forbidden', async () => {
  ...
  expect(
    within(rows[1]).queryByRole("button", { name: "Delete" }),
  ).toBeDefined();
...
test('It is possible to delete not already saved "number of people" field', async () => {
  ...
  await user.type(
    within(screen.getByRole("dialog")).getByRole("textbox", { name: "Label" }),
    "people",
  );
  await user.click(
  ...
  const deleteBtn = await within(rows[1]).findByRole("button", {
    name: "Delete",
  });
