...
test('Navigate template pages', async () => {
  ...
  server.use(
    http.get('/wpoe/v1/admin/events', async () => {
      return HttpResponse.json([
        { "id": 1, "name": "event1", "date": "2030-01-01", "registrations": 3, "postTitle": null, "postPermalink": null, "hasMultipleReferences": false },
        { "id": 2, "name": "event2", "date": "2030-01-05", "registrations": 0, "postTitle": "example", "postPermalink": "http://localhost/?p=42", "hasMultipleReferences": false }
      ]);
    })
  );
  ...
  expect(await screen.findByText(/Registrations for the event "event1"/)).toBeInTheDocument();
