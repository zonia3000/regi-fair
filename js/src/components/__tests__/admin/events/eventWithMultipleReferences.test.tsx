...
test('Open event with multiple references modal', async () => {
  ...
  server.use(
    http.get('/wpoe/v1/admin/events', async () => {
      return HttpResponse.json([
        { "id": 1, "name": "event1", "date": "2030-01-01", "registrations": 0, "postTitle": "example", "postPermalink": "http://localhost/?p=42", "hasMultipleReferences": true }
      ]);
    })
  );
  ...
  expect(within(dialog).getByText('Event form is referenced in multiple posts')).toBeInTheDocument();
