...
test("List registrations with deleted records", async () => {
  ...
  const deletedFieldsCheckbox = screen.getByRole("checkbox", {
    name: "Show deleted fields",
  });
...
test("List registrations with waiting list", async () => {
  ...
  server.use(
    http.get("/regifair/v1/admin/events/1/registrations", ({ request }) => {
      ...
      if (waitingList === "true") {
        return HttpResponse.json({
          eventName: "test",
          head: [{ label: "name", deleted: false }],
          body: [[3, "2024-09-15 20:27:03", "gianna"]],
          total: 1,
          totalParticipants: 2,
          totalWaiting: 1,
        });
  ...
  await act(async () => {
    const confirmedRegistrationsLink = screen.getByRole("button", {
      name: "2",
    });
