...
test('Render optional text field, disabled', async () => {
  ...
  render(<Form disabled={true} admin={false} eventId={1} setLoading={setLoading} loading={loading} />);

  const field = await screen.findByRole('textbox', { name: 'myfield (optional)' });
  expect(field).toBeInTheDocument();
