...
test("Event referenced in posts", async ({ page, context, request }) => {
  ...
  async function closeWelcomeModalIfNeeded() {
    const welcome = await page.getByText("Welcome to the editor").isVisible();
    if (!welcome) {
      return;
    }
  ...
  await test.step("Add third event to one post and then unpublish the post", async () => {
    ...
    await expect(
      page.getByText("Post reverted to draft").first()
    ).toBeVisible();
  });
  ...
  await test.step("Check events references", async () => {
    ...
    await expect(
      page.getByRole("row", { name: eventName1 }).getByRole("cell").nth(3)
    ).toContainText(postTitle1);
    await expect(
      page.getByRole("row", { name: eventName2 }).getByRole("cell").nth(3)
    ).toContainText(postTitle4);
    await page
      .getByRole("row", { name: eventName2 })
      .getByRole("cell")
      .nth(3)
      .getByRole("button")
      .click();
    ...
    await expect(
      page.getByRole("row", { name: eventName3 }).getByRole("cell").nth(3)
    ).toContainText("-");
    await expect(
      page.getByRole("row", { name: eventName4 }).getByRole("cell").nth(3)
    ).toContainText("-");
    await expect(
      page.getByRole("row", { name: eventName5 }).getByRole("cell").nth(3)
    ).toContainText(postTitle7);
    await expect(
      page.getByRole("row", { name: eventName6 }).getByRole("cell").nth(3)
    ).toContainText(postTitle7);
  });

  await test.step("Delete referenced event and open post", async () => {
    await page
      .getByRole("row", { name: eventName1 })
  ...
  await test.step("Delete posts", async () => {
    async function deletePost(postId: number) {
      const response = await request.delete(
        `/index.php?rest_route=/wp/v2/posts/${postId}&force=true`,
        {
          headers: {
            Cookie: cookies,
            "X-WP-Nonce": nonce,
